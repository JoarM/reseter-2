...
import { createApikey } from "@/actions/project";
...
export function CreateApikeyForm({
    ...
    const [form, createAction] = useFormState(createApikey, undefined);
        ...
        <form action={createAction} className="mx-auto max-w-sm w-full">
            ...
            <>
                <span className="text-sm font-medium flex items-center">
                    Api key: {form.key}
                    <button onClick={() => navigator.clipboard.writeText(form.key)}>
                        <Copy
                        className="ml-2"
                        size={16}
                        />
            ...
            <>
                ...
                {form?.error?.name && 
                <span className="text-sm font-medium text-destructive mt-2 block">{form.error.name}</span>}
